    ...
    <div className="h-full flex flex-col bg-[#F8F9FB] animate-in fade-in duration-500 font-sans select-none">
      ...
      <header className="px-8 py-6 bg-white border-b border-slate-200 shrink-0">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl font-extrabold uppercase tracking-tight text-[#5D4037]">Active Order Management</h1>
          ...
          <div className="flex items-center gap-3">
            <div className="h-10 px-4 bg-amber-50 text-[#5D4037] rounded flex items-center gap-2 border border-amber-100 shadow-sm">
              <span className="w-2 h-2 rounded-full bg-[#5D4037] animate-pulse" />
              <span className="text-[10px] font-extrabold uppercase tracking-widest font-black text-[#5D4037]">{orders.length} Orders Active</span>
            </div>
             <button 
              ...
              {filteredOrders.map(order => (
                ...
                >
                   ...
                   <div className="p-4 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                      <div className="flex items-center gap-3">
                         <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-white shadow-lg ${order.status === 'Ready' ? 'bg-emerald-500 shadow-emerald-500/10' : 'bg-[#5D4037] shadow-[#5D4037]/10'}`}>
                            <Utensils size={14} />
                         </div>
                         <div>
                      ...
                      <div className="flex items-center gap-2">
                         <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-full border ${order.status === 'Ready' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-[#5D4037] border-amber-100'}`}>
                            {order.status}
                         </span>
                   ...
                   <div className="p-5 flex-1 space-y-3">
                      ...
                      <div className="pt-4 border-t border-slate-50 flex items-center justify-between">
                         <div className="flex items-center gap-1.5">
                            <Timer size={12} className={order.status === 'Ready' ? 'text-emerald-500' : 'text-[#5D4037]'} />
                   ...
                   <div className="p-4 bg-slate-50/50 border-t border-slate-100 flex gap-2">
                      ...
                      <button 
                        ...
                        className={`flex-1 py-2.5 rounded-lg text-[9px] font-black uppercase tracking-widest text-white transition-all shadow-md active:scale-95 ${order.status === 'Ready' ? 'bg-[#5D4037] hover:opacity-90 shadow-[#5D4037]/10' : 'bg-emerald-500 hover:bg-emerald-600 shadow-emerald-500/10'}`}
                      >
...
function TabBtn({ active, onClick, label, count }) {
  return (
    <button 
      onClick={onClick}
      className={`pb-3 text-[10px] font-black uppercase tracking-[0.2em] relative transition-all flex items-center gap-2 ${active ? 'text-[#5D4037]' : 'text-slate-400 hover:text-slate-600'}`}
    >
      {label}
      {count > 0 && <span className={`px-1.5 py-0.5 rounded text-[8px] ${active ? 'bg-[#5D4037] text-white' : 'bg-slate-100 text-slate-400'}`}>{count}</span>}
      {active && <motion.div layoutId="activeTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#5D4037]" />}
    </button>
  );
}
