...
export function KdsOrderCard({ order, onClick }) {
  ...
  const getStatusInfo = (status) => {
    switch (status) {
      case 'new': return { color: 'bg-blue-500', text: 'New', border: 'border-blue-500/20' };
      case 'preparing': return { color: 'bg-orange-500', text: 'Preparing', border: 'border-orange-500/20' };
      case 'delayed': return { color: 'bg-red-500', text: 'Delayed', border: 'border-red-500/40' };
      case 'ready': return { color: 'bg-emerald-500', text: 'Ready', border: 'border-emerald-500/20' };
      default: return { color: 'bg-slate-500', text: status, border: 'border-slate-500/20' };
    }
  };

  const statusInfo = getStatusInfo(order.status);
  const effectiveStatusColor = isDelayed ? 'bg-red-500' : statusInfo.color;
  ...
  return (
    <motion.div
      ...
      className={`border rounded-3xl overflow-hidden cursor-pointer flex flex-col h-full shadow-2xl transition-all duration-300 ${
        isDarkMode 
          ? `bg-slate-900 ${statusInfo.border}` 
          : `bg-white border-slate-200 hover:border-slate-300`
      }`}
    >
      {/* Header */}
      <div className={`${effectiveStatusColor} px-4 md:px-6 py-3 md:py-5 flex items-center gap-3 overflow-hidden`}>
         <div className="flex-shrink-0 w-9 h-9 md:w-12 md:h-12 bg-white/20 border border-white/10 rounded-xl flex items-center justify-center text-white backdrop-blur-sm">
         ...
         {order.items.map((item) => (
            <div key={item.id} className="flex items-start gap-4 md:gap-6">
               <div className={`flex items-center justify-center w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-xl font-black text-xs md:text-base border transition-colors ${
                 isDarkMode ? 'bg-slate-800 text-teal-400 border-white/5' : 'bg-slate-100 text-slate-600 border-slate-200'
               }`}>
                  {item.quantity}x
               </div>
               <div className="flex-1 min-w-0">
                  ...
                  {item.note && (
                    <div className="mt-1 md:mt-2 flex items-start gap-2 text-rose-400">
      ...
      <div className={`px-6 md:px-8 py-4 md:py-6 border-t flex items-center justify-between mt-auto text-sm md:text-base transition-colors ${
        isDarkMode ? 'bg-black/40 border-white/5' : 'bg-slate-50 border-slate-100'
         ...
         <div className={`flex items-center gap-2 font-mono text-lg md:text-2xl font-black transition-colors ${
           isDelayed ? 'text-red-400' : (isDarkMode ? 'text-teal-400' : 'text-teal-600')
         }`}>
