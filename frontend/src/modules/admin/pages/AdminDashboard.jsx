...
import {
   ...
   RefreshCcw, ChevronRight, TrendingUp, ArrowRight, Calendar,
    TrendingDown, Box, PieChart, Users, Building2, Trash2, Bell, Eye
} from 'lucide-react';
import {
         ...
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7 gap-4">
               ...
               >
                  <div className="flex items-center justify-between mb-3">
                     <div className="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
               ...
               >
                  <div className="flex items-center justify-between mb-3">
                     <div className="w-8 h-8 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
                        <ShoppingBag size={16} strokeWidth={2.5} />
                     </div>
                  </div>
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Orders</p>
                  <div className="text-xl font-black text-slate-900 leading-none mb-2">{(orders?.todayDineIn || 0) + (orders?.todayTakeaway || 0)}</div>
                  <div className="flex gap-2">
                     <div className="flex flex-col">
                        <span className="text-[9px] font-black text-blue-600 leading-none">{orders?.todayDineIn || 0}</span>
                        <span className="text-[6px] font-black text-slate-400 uppercase">Dine-In</span>
                     </div>
                     <div className="flex flex-col border-l border-slate-100 pl-2">
                        <span className="text-[9px] font-black text-amber-500 leading-none">{orders?.todayTakeaway || 0}</span>
               ...
               >
                  <div className="flex items-center justify-between mb-3">
                     <div className="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
               ...
               >
                  <div className="flex items-center justify-between mb-3">
                     <div className="w-8 h-8 rounded-lg bg-orange-50 text-orange-600 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
               ...
               >
                  <div className="flex items-center justify-between mb-3">
                     <div className="w-8 h-8 rounded-lg bg-slate-900 text-white flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
                        <CheckCircle2 size={16} strokeWidth={2.5} />
                     </div>
                  </div>
               ...
               >
                  <div className="flex items-center justify-between mb-3">
                     <div className="w-8 h-8 rounded-lg bg-rose-50 text-rose-600 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
               ...
               <div className="bg-white border border-slate-200 p-8 rounded-[2rem] shadow-sm flex flex-col">
                  <div className="flex flex-col mb-8 pb-4 border-b border-slate-50">
                     <div className="flex items-center justify-between mb-4">
                        <h3 className="text-sm font-black text-slate-900 uppercase tracking-[0.1em]">Live Stream</h3>
                        <div className="flex items-center gap-2">
                           <span className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
                           <span className="text-[9px] font-black text-slate-400 uppercase underline decoration-blue-500/30 underline-offset-4">Operational Pulse</span>
                        </div>
                     </div>

                     <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 mb-4 overflow-x-auto no-scrollbar">
                        {[
                           { id: 'dashboard', label: 'All', icon: LayoutGrid },
                           { id: 'incoming', label: 'New', icon: Bell },
                  ...
                  <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                     {recentOrders?.filter(o => {
                        ...
                        // Operational Stream Filter
                        if (view === 'incoming') return st === 'pending' || st === 'reserved';
                        if (view === 'preparing') return st === 'preparing';
                        if (view === 'completed') return ['completed', 'served', 'paid', 'settled'].includes(st);
                        if (view === 'cancelled') return st === 'cancelled';
                        ...
                        >
                           ...
                           <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-3">
                                 <span className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${['paid', 'Paid', 'settled', 'Settled'].includes(order.status) ? 'bg-emerald-50 text-emerald-600' : 'bg-orange-50 text-orange-600'
                                    }`}>
                                    {order.status}
                                 </span>
...
function DashboardSummaryBtn({ label, count, active, onClick, color }) {
   return (
      ...
      >
         <p className={`text-[8px] font-black uppercase tracking-widest mb-1 ${active ? 'text-slate-400' : 'text-slate-400'}`}>{label}</p>
         <p className={`text-sm font-black ${active ? 'text-white' : 'text-slate-900'}`}>{count || 0}</p>
      </button>
   );
}
