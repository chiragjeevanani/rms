...
export default function TopBar() {
  ...
  useEffect(() => {
    ...
    fetchAdminProfile(token, controller.signal)
      .then(data => {
        setAdminData(data);
             ...
             {showNotifications && (
               ...
               >
                   <div className="p-3 bg-[#222222] border-b border-white/8 flex items-center justify-between">
                      ...
                      <button type="button" onClick={() => setShowNotifications(false)} aria-label="Close notifications" className="text-white/40 hover:text-white transition-colors">
                        <X size={14} />
                      </button>
                   </div>
...
const fetchAdminProfile = async (token, signal) => {
